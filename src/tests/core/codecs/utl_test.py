...
class TestUtils(unittest.TestCase):
    ...
    def test_29_datetime_parse(self):
        """Test `parse` on `datetime`."""

        now = fqr.core.lib.datetime.datetime.now(
            fqr.core.lib.datetime.timezone.utc
            )
        self.assertEqual(
            now,
            fqr.core.codecs.utl.parse(
                fqr.core.codecs.lib.json.dumps(now),
                fqr.core.lib.datetime.datetime
                )
            )

    def test_30_anti_datetime_parse(self):
        """Test `parse` on `datetime`."""

        self.assertEqual(
            fqr.core.codecs.enm.ParseErrorRef.datetime_decode,
            fqr.core.codecs.utl.parse(
                'asdf',
                fqr.core.lib.datetime.datetime
                )
